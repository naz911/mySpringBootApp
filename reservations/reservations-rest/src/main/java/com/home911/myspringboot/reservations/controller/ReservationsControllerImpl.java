...
public class ReservationsControllerImpl {
	...
	@RequestMapping(method = RequestMethod.PATCH, value = "/{reservationUuid}", consumes = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(value = HttpStatus.OK)
	...
	private void checkReservationDuration(ReservationWS reservationWS) {
		if (DAYS.between(reservationWS.getFromDate(), reservationWS.getToDate()) > MAX_DURATION) {
